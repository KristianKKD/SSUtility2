using System;
using System.Collections.Generic;
using System.IO.Ports;
using System.Linq;
using System.Text;
using System.Threading.Tasks;

namespace SSUtility2 {
    public class SerialWriter {

        SerialPort sp;

        static readonly uint[] crc_table = new uint[256] {
                  
                  };

        public void MakeSerialPort(string port, int baud) {
            if (sp != null) {
                Disconnect();

            }

            sp = new SerialPort(port, baud);
        }

        public void Write(byte[] code, string data) {
            string cmd1 = "";

            if (code[0] != 0)
                cmd1 += code[0].ToString("X2");
            else
                cmd1 += "";

            if (!sp.IsOpen)
                return;

            string finalCode = "\x02" + ((int)1).ToString("X2") + cmd1 + code[1].ToString("X2") + data + "\x03";
            byte[] byteCode = Encoding.UTF8.GetBytes(finalCode);


            sp.Write(finalCode);
            sp.Write(SSCPChecksum(byteCode, byteCode.Length).ToString("X2"));

        }

        public void Read() {

        }

        public void Disconnect() {
            if (sp.IsOpen) {
                sp.Close();
            }
            sp.Dispose();
            sp = null;
        }

        private int SSCPChecksum(byte[] cmd, int len) {
            int i;
            int rv = 0;
            if (len < 3 || cmd[0] != 0x02 || cmd[len - 1] != 0x03)
                return rv;
            for (i = 1; i < len - 1; i++) {
                rv = (int)crc_table[rv ^ (int)cmd[i]];
            }
            return rv;
        }

        private int Char2Hex(char c) {
            int rv;
            rv = "0123456789ABCDEF".IndexOf(Char.ToUpper(c));
            return rv;
        }

        private int HexByte(char c1, char c2) {
            int rv;
            rv = Char2Hex(c1) * 16 + Char2Hex(c2);
            return rv;
        }

        //private int SSCPHexValue(int start, int length) {
        //    int rv = 0;
        //    int i;
        //    length += start;
        //    for (i = start; i < length; i++)
        //        rv = (rv << 4) + Char2Hex((char)replybuf[i]);
        //    return rv;
        //}


    }

}
